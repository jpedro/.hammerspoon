...
-- hs.alert.show("pomodoro")

-- Pomodoro module
local pom_work_period_sec  = 1 * 60
local pom_rest_period_sec  = 1 * 60
...
local function pom_update_display()
  ...
  -- local str = string.format ("%s %d %02d", pom_curr_active_type, time_granular, pom_work_count)
  local str = string.format ("%s %d:%02d", pom_curr_active_type, time_min, time_sec)
